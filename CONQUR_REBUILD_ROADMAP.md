...
2. **AI: approved architecture, not yet implemented.** Supabase Edge Functions + Anthropic API, but built behind a provider-independent `AIService` interface so the model is swappable infrastructure, never a visible brand element. Haiku is the default for both onboarding reflection and ordinary "Let's Talk" support; escalate to Sonnet only for harder/more nuanced turns. **No AI code ships in Phase 1.**
...
*Does not start until Phase 1 + Phase 2 are reviewed and working end-to-end.*
- Build the `AIService` interface first — model-agnostic, so "Claude" never appears in the product surface, only "Conqur."
- Supabase Edge Function proxy; Haiku as default for onboarding interpretation and ordinary Let's Talk turns; explicit escalation path to Sonnet for harder/more nuanced conversations (needs a concrete trigger — e.g. detected distress language, explicit user request for more depth, or a low-confidence flag from Haiku itself).
- Hard-coded keyword/pattern safety pre-filter for substance dependence, disordered eating, self-harm, crisis language → routes to professional-support copy independent of model output, especially relevant once eating/drinking Quests ship in Phase 5.
...
1. **Exact escalation trigger for Haiku → Sonnet** (Phase 3) — needs a concrete rule before implementation, not just "harder requests."
2. **Exact copy for the Main Quest demotion moment** (Phase 1 risk #2) — worth drafting and reviewing before it ships, given how central "don't feel punitive" is to the whole doc.
3. **Curated content for the two validation Quests** — the doc's own worked examples (Section 8-9) are a strong starting draft; confirm before I write them into `app.js`, or hand me refined versions.
